  ...
  #Strecth布局      
    ...
    def capture_frame(self):
        ...
        if self.video_capture is not None:
            ...
            if ret:
                ...
                # 计算裁切区域
                x_start = 394  # 左边裁切394像素
                x_end = width - 22  # 右边裁切22像素
                y_start = 0  # 从顶部开始
                y_end = height - 56  # 底部裁切56像素
                
                # 执行裁切
                cropped_frame = frame[y_start:y_end, x_start:x_end]
                
                # 调整大小为1024x1024
                self.image = cv2.resize(cropped_frame, (1024, 1024))
                
                # 显示在original_image_label中
                self.display_image(self.original_image_label, self.image)
                self.status_bar.showMessage(f"已捕获并裁切当前帧 (1024x1024)")
    ...
    def load_dnn_sr_model(self, method, scale):
        model_paths = {
            ...
            "FFSCN": f"./model/FFSCN_x{scale}.pb",
...
class VTKReconstructionTab(QWidget):
    def __init__(self, status_bar):
        ...
        self.reader = None
        # 图标设置
        icon_path = "Platform\resouce\imris.ico"
...
class MainApp(QMainWindow):
    ...
    def load_style(self):
        """加载样式表"""
        try:
            with open("Platform\resource\style.qss", "r") as f:
                style = f.read()
